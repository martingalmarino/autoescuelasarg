...
export default function AddSchoolForm({ onClose, onSuccess }: AddSchoolFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.name || !selectedProvince || !formData.cityId) {
        setError('Nombre, provincia y ciudad son requeridos')
        setLoading(false)
        return
      }
      ...
      if (data.success) {
        alert('✅ Autoescuela agregada correctamente!')
        onSuccess()
        onClose()
      } else {
        setError(data.error || 'Error al agregar autoescuela')
      }
    } catch (error) {
      ...
      <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold">Agregar Nueva Autoescuela</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <Button type="submit" disabled={loading}>
                {loading ? 'Agregando...' : 'Agregar Autoescuela'}
              </Button>
            </div>
          </form>
        </div>
